...
public:
   ...
   /* Model* createCustomModel(std::string objPath, std::string texturePath, glm::vec3 position, glm::vec3 rotation, glm::vec3 scale, Camera *camera) {
    ...
    //Custom Object with texture + specular Texture
    Model* createCustomModel(std::string objPath, std::string texturePath, std::string specularTexturePath, glm::vec3 position, glm::vec3 rotation, glm::vec3 scale) {
        Utils* utils = new Utils(device, physicalDevice, commandPool, graphicsQueue);
        Model* obj = new CreateObject(objPath, texturePath, specularTexturePath, textureSpecularShader, position, rotation, scale, utils);
        sceneObjects.push_back(obj);
        return obj;
    }

    //Custom Object with texture + Emission Texture
    Model* createCustomModel(std::string objPath, std::string texturePath, std::string emissionTexturePath, glm::vec3 position, glm::vec3 rotation, glm::vec3 scale, bool isEmissionTexture) {
        Utils* utils = new Utils(device, physicalDevice, commandPool, graphicsQueue);
        Model* obj = new CreateObject(objPath, texturePath, emissionTexturePath, textureSpecularShader, position, rotation, scale, utils, true);
        sceneObjects.push_back(obj);
        return obj;
    }

    //Custom Object with texture + Specular Texture + Emission Texture
    Model* createCustomModel(std::string objPath, std::string texturePath, std::string specularTexturePath, std::string emissionTexturePath, glm::vec3 position, glm::vec3 rotation, glm::vec3 scale) {
        Utils* utils = new Utils(device, physicalDevice, commandPool, graphicsQueue);
        Model* obj = new CreateObject(objPath, texturePath, specularTexturePath, emissionTexturePath, textureSpecularShader, position, rotation, scale, utils);
        sceneObjects.push_back(obj);
        return obj;
    }
...
private:
    ...
    void initVulkan() {
        ...
        textureSpecularShader = new TextureMapsShader(swapChainExtent, device, renderPass, msaaSamples, true);
        ...
        colorShader = new ColorShader(swapChainExtent, device, renderPass, msaaSamples);
        lightSourceShader = new LightSourceShader(swapChainExtent, device, renderPass, msaaSamples);
    ...
    void recreateSwapChain() {
        ...
        textureSpecularShader->createGraphicsPipeline(swapChainExtent, device, renderPass, msaaSamples);
        ...
        colorShader->createGraphicsPipeline(swapChainExtent, device, renderPass, msaaSamples);
        lightSourceShader->createGraphicsPipeline(swapChainExtent, device, renderPass, msaaSamples);
        createColorResources();
        createDepthResources();
        createFramebuffers();
    ...
    void updateUniformBuffer(Model* obj, uint32_t currentImage) {
        ...
        ubo.material.emissionColor = obj->getEmissionColor();
        ubo.material.emissionRate = obj->getEmissionRate();
