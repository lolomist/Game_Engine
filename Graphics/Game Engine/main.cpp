...
int main() {
    ...
    engine.setDirectionalLight(glm::vec3(-0.2f, -1.0f, -0.3f), glm::vec3(0.001f, 0.001f, 0.001f), glm::vec3(0.20f, 0.19f, 0.62f), glm::vec3(0.5f, 0.5f, 0.8f));

    engine.createPointLight(glm::vec3(0.7f, 0.2f, 2.0f), glm::vec3(1.0f, 0.6f, 0.0f), 1.0f, 0.09f, 0.032f, true);
    ...
    engine.createCube("textures/cube.png", "textures/matrix.jpg", glm::vec3(-15.f, 10.f, 0.f), glm::vec3(0.f), glm::vec3(2.f), true);


    engine.createCube("textures/batimentsTexture.png", "textures/cube_specular3.png","textures/neon1.png", glm::vec3(50.f, 10.f, 0.f), glm::vec3(0.f), glm::vec3(20.f));
   ...
   /* for (unsigned int i = 0; i < 10; i++) {
    ...
    engine.createCustomModel("models/game_scene.obj", glm::vec3(1.0f), glm::vec3(0.f, 0.f, -80.f), glm::vec3(0.f), glm::vec3(1.f));
    ...
    while (engine.isWindowOpen()) {
      ...
      /*  glm::vec3 lightColor;
        ...
        if (win->isKeyPressed(GLFW_KEY_W)) {
            ...
            camera->move(5.f, FORWARD, dt);
        }
        if (win->isKeyPressed(GLFW_KEY_A)) {
            //obj->move(5.f, LEFT, dt);
            camera->move(5.f, LEFT, dt);
        }
        if (win->isKeyPressed(GLFW_KEY_S)) {
            //obj->move(5.f, BACKWARD, dt);
            camera->move(5.f, BACKWARD, dt);
        }
        if (win->isKeyPressed(GLFW_KEY_D)) {
            //obj->move(5.f, RIGHT, dt);
            camera->move(5.f, RIGHT, dt);
        }
        if (win->isKeyPressed(GLFW_KEY_SPACE)) {
            //obj->move(5.f, UP, dt);
            camera->move(5.f, UP, dt);
        }
        ...
        if (win->isKeyPressed(GLFW_KEY_LEFT_CONTROL)) {
            //obj->move(5.f, DOWN, dt);
            camera->move(5.f, DOWN, dt);
        }
